...
Definition esp  mu f   := fine (sum (EFin \o (fun x => f x * mu x))).
Definition espc mu f A := fine (sum (EFin \o (fun x => f x * prc mu (pred1 x) A))).
...
rewrite /esum ge_ereal_sup//= => _ [X [finX _]] <-.
...
+ rewrite /dlet -lee_fin.
  ...
  rewrite (eq_esum (b:= fun x  => `|(EFin \o f i) x|%E)) //.
...
rewrite (eq_esum (b := fun y => \esum_(x in [set: T]) (S y x)%:E)%E).
...
rewrite (eq_esum (b := fun x => \esum_(y in [set: U]) (S y x)%:E)%E).
+ move => x ?; subst S; rewrite EFinM fineK ?fin_esum //.
  + rewrite -esumZ // ?lee_tofin // => [? |].
    + by rewrite EFinM lee_tofin // mulr_ge0.
    + by apply eq_esum => ??; rewrite -EFinM mulrA.
by rewrite esum_esum' // => ??; rewrite /S EFinM lee_tofin // !mulr_ge0.
Qed.
End DLetDLet.
...
suff key : forall (X' Y' : Type) (g : X' -> Y' -> \bar R) (C : set X') (D : set Y'),
    (ereal_sup [set ereal_sup [set g x y | y in D] | x in C] <=
      ereal_sup [set ereal_sup [set g x y | x in C] | y in D])%E.
...
Lemma esupZl {R : realType} (c : \bar R) (X : set \bar R):
  0 <= c ->
  X != set0 ->
  (forall x, X x -> 0 <= x) ->
...
Section ereal_sup_range.
  Local Open Scope ereal_scope.
  Local Open Scope classical_set_scope.

Lemma esup_add {R : realType} (u v : nat -> \bar R) :
...
Lemma esupZl_range {R : realType} {T: choiceType}
  (f : T -> nat -> \bar R)
  (c : \bar R) (x : T) (cpos : 0 <= c) :
  (forall t n, 0 <= f t n) ->
  (c * ereal_sup (range (f x)) = ereal_sup (range (fun n => c * f x n)))%E.
Proof.
move => fpos.
have <- :
...
Lemma ereal_sup_sum_seq (l : seq T) :
...
Proof.
rewrite /esum.
under eq_imagel => B [fin ?] do rewrite fsbig_finite// ereal_sup_sum_seq.
...
Lemma sup_range_lim (u : (\bar R)^nat) :
...
rewrite (@esupZl_range R T (fun t n => (nf n t)%:E)) //=.
...
rewrite (@esum_esup_comm R T (fun x n => (f n x * h x y)%:E)) /=.
...
rewrite (@esum_esup_comm R T (fun t n => (reduce t n z * mu t)%:E)) /=.
...
Proof.
rewrite /pr.
rewrite (eq_esum (b := fun y => if x == y then (mu y)%:E else 0)).
+ move=> y _ /=; rewrite [y == x]eq_sym.
  by case: ifP => xy; rewrite ?xy /= ?mul1r ?mul0r.
by rewrite esum_unit esum_set1; first by rewrite lee_fin.
Qed.

(* -------------------------------------------------------------------- *)
Lemma pr_exp mu (E : pred T) : \P_[mu] E = \E_[mu] (fun m => (E m)%:R).
Proof.
rewrite /pr /esp esum_sum' //= => x.
by rewrite lee_tofin // mulr_ge0 // ler0n.
Qed.
...
Proof.
rewrite /pr.
rewrite (eq_esum (b := fun y => if x == y then ((E y)%:R)%:E else 0)).
+ move=> y _ /=; rewrite dunit1E.
  by case: ifP => xy; rewrite ?xy /= ?mulr1 ?mulr0.
by rewrite esum_unit esum_set1; first by rewrite lee_fin ler0n.
Qed.

Lemma exp_dunit (f : T -> R) (x : T) : \E_[dunit x] f = f x.
Proof.
rewrite /esp.
rewrite (@eq_sum _ _ _ (fun y : T => if x == y then (f y)%:E else 0)%E).
...
rewrite pr_predT /esp; congr fine.
rewrite esum_sum'.
+ by move => ?;rewrite lee_tofin // mulr_ge0.
apply/eq_esum => //=.
...
rewrite (esum.eq_sum (S2 := fun x => (A x * (mu x)%:E - B x * (mu x)%:E)%E)).
...
Proof.
by rewrite /esp /espe; congr (fine _); apply: esum.eq_sum => x /=; rewrite EFinM.
Qed.

Lemma espeEFin mu (g : T -> R) :
  espe mu (EFin \o g) = sum (EFin \o (fun x => g x * mu x)).
Proof. by rewrite /espe; apply: esum.eq_sum => x /=; rewrite EFinM. Qed.
...
congr (_ * _)%E.
rewrite /esp fineK//.
exact: (summable_sum_fin_num (sg (nu _))).
Qed.
...
Proof.
rewrite /pr /esp esum_sum'.
+ by move=> x; rewrite lee_fin; apply: mulr_ge0; [exact: ge0_pr|exact: ge0_mu].
congr (fine _).
rewrite {1}(eq_esum (b:= fun t => \esum_(x in [set: U]) ((E t)%:R * (mu x * f x t))%:E)%E).
...
- exact: (summable_sum_fin_num sm1).
- exact: (summable_sum_fin_num sm2).
- apply: summable_le_sum; first exact: sm2.
...
have hpr1 : forall x, \P_[mu] [predI pred1 x & A] = (A x)%:R * mu x.
  move=> x; rewrite /pr.
  rewrite (eq_esum (b := fun y => if x == y then ((A y)%:R * mu y)%:E else 0)).
    move=> y _ /=; rewrite !inE [y == x]eq_sym.
    by case: ifP => xy; rewrite ?xy /= ?mul1r ?mul0r.
  rewrite esum_unit esum_set1; first by rewrite lee_fin; exact: a0.
  by [].
rewrite (eq_esum (b := fun x => ((\P_[mu] A)^-1 * ((A x)%:R * mu x))%:E)).
...
rewrite (eq_esum (b := fun y => if x == y then ((A y)%:R * mu y)%:E else 0)).
  move=> y _ /=; rewrite !inE [y == x]eq_sym.
  by case: ifP => xy; rewrite ?xy /= ?mul1r ?mul0r.
rewrite esum_unit esum_set1.
  by rewrite lee_fin; apply: mulr_ge0; [exact: ler0n|exact: ge0_mu].
by [].
Qed.
...
Lemma sum_fin_num_mu_f f mu (P : pred T):
    \E?_[mu] f ->
    sum (EFin \o (fun x => (P x)%:R * (f x * mu x))) \is a fin_num.
Proof.
  move=> ?; apply: summable_sum_fin_num;  exact: summable_pr_f.
Qed.

Lemma pr_esp_sum mu f (P : pred T):
  \E?_[mu] f ->
  \P_[mu] P * \E_[mu, P] f =
  fine (sum (EFin \o (fun x => (P x)%:R * (f x * mu x)))).
Proof.
move => sm.
  have [c0|cn0] := eqVneq (\P_[mu] P) 0.
rewrite c0 mul0r.
  suff -> : sum (EFin \o (fun x => (P x)%:R * (f x * mu x))) = 0 by rewrite fine0.
  rewrite (esum.eq_sum (S2 := fun=> 0%E)) ?esum.sum0// => x.
  suff h0 : (P x)%:R * mu x = 0 :> R by rewrite /= mulrCA h0 mulr0.
  case Hpx: (P x); last by rewrite mul0r.
  by rewrite (pr_eq0 c0 Hpx) mulr0.
rewrite /espc.
rewrite (esum.eq_sum (S2 := fun x => ((\P_[mu] P)^-1 %:E *
                (EFin \o (fun x => ((P x)%:R * (f x * mu x))%R)) x)%E)).
  move=> x; rewrite /= prc_pred1 -EFinM; congr (_ %:E).
  by rewrite mulrA mulrC; congr (_ * _); exact: mulrCA.
rewrite (esum.summable_sumZ _ (summable_pr_f P sm));
...
congr fine; rewrite -(sumD (summable_pr_f A sm) (summable_pr_f (predC A) sm)).
apply: esum.eq_sum => x /=; rewrite -EFinD; congr (_ %:E).
...
rewrite (eq_esum (b:= fun x=> `|(F x)%:E * (mu x)%:E|)%E).
...
Proof.
move=> g0; rewrite /esp; apply: fine_ge0; apply: esum.sum_ge0 => x.
...
rewrite (esum.eq_esum (b := EFin \o (fun x => esp (nu x) g * mu x))).
+ move=> x _ /=; rewrite ger0_norm //.
rewrite -(esum_sum' (S := EFin \o (fun x => esp (nu x) g * mu x))).
+ by move=> x; rewrite lee_fin; exact: hge0.
rewrite -espeEFin -(eexp_dlet_esp mu nu g0 sg) espeEFin.
exact: (summable_sum_fin_num (sg (dlet nu mu))).
Qed.
...
move=> sf sg.
rewrite !espE -fineB //.
+ by rewrite espeEFin; exact: (summable_sum_fin_num sf).
+ by rewrite espeEFin; exact: (summable_sum_fin_num sg).
congr (fine _).
...
Proof.
move=> hE h.
rewrite (eq_esum (b:=(fun x : T => ((E x)%:E * ((dlim f) x)%:E)%E))).
...
rewrite -(@esum_esup_comm R T (fun x n => ((E x)%:R * f n x)%:E)) //.
+ by move=> x n; rewrite lee_fin; apply: mulr_ge0; [exact: ler0n | exact: ge0_mu].
apply: eq_esum => x _.
rewrite EFinM distr_lub_sup (@esupZl_range R T (fun a b => (f b a)%:E)) //.
+ by move=> t n; rewrite lee_fin; exact: ge0_mu.
Qed.
